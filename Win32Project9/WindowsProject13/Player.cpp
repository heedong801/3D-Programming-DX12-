...
CPlayer::CPlayer::CPlayer(int nMeshes) : CGameObject(nMeshes)
{
	m_pCamera = NULL;
...
void CPlayer::Rotate(float x, float y, float z)
{
	...
	else if (nCameraMode == SPACESHIP_CAMERA)
	{
		...
		if (z != 0.0f)
		{
			...
			m_xmf3Up = Vector3::TransformNormal(m_xmf3Up, xmmtxRotate); m_xmf3Right = Vector3::TransformNormal(m_xmf3Right, xmmtxRotate);
		}
	}
...
{
	m_xmf3Position = XMFLOAT3(0, 0, -400);
...
void CTerrainPlayer::OnPlayerUpdateCallback(float fTimeElapsed) 
{
	...
	그리고 플레이어 메쉬의 높이가 12이고 
	플레이어의 중심이 직육면체의 가운데이므로
