...
void CScene::CheckPlayerByBulletCollisions()
{
	for (int i = 0; i < 5; i++)
	{
		if (m_pBossBulletObject[i].m_xmOOBB.Intersects(m_pPlayer->m_xmOOBB))
		{
			GameReset();
...
{
	if (m_pBossObject->m_xmOOBB.Intersects(m_pPlayer->m_xmOOBB))
	{
		GameReset();
	}
}
...
{			
	...
	{
		if (m_bMobJen && m_nBossAttackStart < m_pBossObject->m_nBossAttackTimer)
		{
			m_nBossAttackStart++;
			m_bMobJen = false;
		}
		if (m_nBossAttackStart >= m_pBossObject->m_nBossAttackTimer)
		{
