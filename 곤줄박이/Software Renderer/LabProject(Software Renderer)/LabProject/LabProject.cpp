                     ...
                     int       nCmdShow)
{
	...
	while (1) 
	{
		if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE))	//GetMessage는 메시지가 생길때까지 기다림, but 이건 메세지가 있으면 처리하되, 없으면 다른 일
		{												//메세지가 있으면 트루 반환 없으면 폴스 반환
			if (msg.message == WM_QUIT) break;			//들어온 메세지가 종료 메세지일경우 루프 종료
			if (!TranslateAccelerator(msg.hwnd, hAccelTable, &msg))	//키입력을 -> 커맨드로 가기전에 여기서 처리 -> 액셀레이터(기능키?)면 트루 아니면 거짓
			{
		...
		{
			gGameFramework.FrameAdvance();			//프레임워크 시작
		}
	}
	...
	switch (message)
	{
        case WM_SIZE:
            ...
            else
            {
				...
				gGameFramework.ResetDisplay(LOWORD(lParam), HIWORD(lParam));
            } 
			break;
        case WM_LBUTTONDOWN:
            SetCapture(hWnd);
