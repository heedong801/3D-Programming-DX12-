		...
		case WM_KEYUP:
			switch (wParam)
			{
						...
						for (int i = 0; i < m_pScene->m_pShaders[0].m_nObjects; i++)
						{
							...
							if (pExplosiveObject->m_bDeathFlag == false)
							{
								if (Vector3::Length(Vector3::Subtract(m_pPlayer->m_xmf3Position, m_pScene->m_pShaders[0].m_ppObjects[i]->GetPosition())) <= 100)
								{
									...
									pExplosiveObject->m_bZBoomFlag = true;
				...
				case VK_F10:
					break;
				case VK_F4:
					if (m_pScene->m_bCheckMorning)
						m_pScene->m_bCheckMorning = false;
					else
						m_pScene->m_bCheckMorning = true;
						break;
				default:
					break;
			}
...
void CGameFramework::ProcessInput()
{
	...
	if (!bProcessedByScene)
	{
		...
		if (pKeysBuffer[0x51] & 0xF0)
			dwDirection |= DIR_UP;
		if (pKeysBuffer[0x45] & 0xF0)
