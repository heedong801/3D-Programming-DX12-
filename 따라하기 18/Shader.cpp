...
void CObjectsShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	float fHalfWidth = 25.0f, fHalfHeight = 25.0f, fHalfDepth = 250.0f;
	CWallMeshIlluminated *pWallMesh = new CWallMeshIlluminated(pd3dDevice, pd3dCommandList, fHalfWidth * 4.0f, fHalfHeight * 4.0f, fHalfDepth * 4.0f);
	...
	m_pWallsObjects->SetPosition(0.0f, 0.0f, 0.0f);			// 분명 벽의 위치를 0,0,0으로 했는데 큐브오브젝트의 맨마지막에 설정된녀석이랑 똑같은 곳에서 돌고있음.
	m_pWallsObjects->SetMaterial(2);
...
{
	m_pWallsObjects->ReleaseUploadBuffers();
	if (m_ppObjects)
	{
		for (int j = 0; j < m_nObjects; j++) 
